...
public class ChatClient extends Applet implements ActionListener, Runnable
{
  ...
  {
    out.println("LOGIN|"+ name.getText());		// 초기 서버에 접속한 후 접속 메시지를 보냄
    out.flush();
  ...
  {												// 엔터가 들어왔을 경우 실행
	  ...
	  if(e.getSource() == connect) {			// connect 버튼이 눌렸을 경우 서버에 연결
		 try{
		  ...
		  String data= input.getText();	// input 텍스트필드의 값을 읽어서
		  input.setText("");

		  
		  if(data.contains(">")){
			  StringTokenizer st = new StringTokenizer(data, ">"); // 규칙에 따라 받은 메시지를 분리하여 확인
			  String a = st.nextToken(); // 메시지의 받는 대화명
			  String b = st.nextToken(); // 메시지의 내용 부분
			  String c = name.getText(); // 자기자신이 대화명
			  out.println("귓속말|"+ a + "|" + c + "|[" + c + "] -> [" + a + "] : "+ b );
